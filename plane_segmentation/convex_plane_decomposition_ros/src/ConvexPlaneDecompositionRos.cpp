...
ConvexPlaneExtractionROS::ConvexPlaneExtractionROS(ros::NodeHandle& nodeHandle) : tfBuffer_(), tfListener_(tfBuffer_) 
{
  ...
  if (parametersLoaded) 
  {
    ...
    //! grid_map
    filteredmapPublisher_ = nodeHandle.advertise<grid_map_msgs::GridMap>("filtered_map", 1);  // A grid map message to visualize the segmentation and some intermediate results

    boundaryPublisher_ = nodeHandle.advertise<visualization_msgs::MarkerArray>("boundaries", 1);  // A set of polygons that trace the boundaries of the segmented region

    insetPublisher_ = nodeHandle.advertise<visualization_msgs::MarkerArray>("insets", 1); // A set of polygons that are at a slight inward offset from boundaries

    //! tf in worldframe
    regionPublisher_ = nodeHandle.advertise<convex_plane_decomposition_msgs::PlanarTerrain>("planar_terrain", 1); // A custom message that contains the full segmentation as a map together with the boundary information.
  }
}
...
bool ConvexPlaneExtractionROS::loadParameters(const ros::NodeHandle& nodeHandle) {
  //! '/elevation_mapping/elevation_map_raw' -> /elevation_mapping/elevation_map_filter
  ...
  if (!nodeHandle.getParam("target_frame_id", targetFrameId_)) {  //odom
  ...
  if (!nodeHandle.getParam("submap/width", subMapWidth_)) { // 8
    ROS_ERROR("[ConvexPlaneExtractionROS] Could not read parameter `submap/width`.");
    return false;
  }
  if (!nodeHandle.getParam("submap/length", subMapLength_)) { // 8
    ROS_ERROR("[ConvexPlaneExtractionROS] Could not read parameter `submap/length`.");
    return false;
  }
  if (!nodeHandle.getParam("publish_to_controller", publishToController_)) {  // true
...
void ConvexPlaneExtractionROS::callback(const grid_map_msgs::GridMap& message) 
{
  ...
  //! Publish terrain, Publish planar_terrain
  if (publishToController_) 
  {
    regionPublisher_.publish(toMessage(planarTerrain)); //! publish /convex_plane_decomposition/planar_terrain
  }
  ...
  // planarTerrain.planarRegions.bbox2d
