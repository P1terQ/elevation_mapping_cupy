      ...
      isGridmapUpdated_(false) 
{
  ...
  nh.param<std::vector<std::string>>("pointcloud_topics", pointcloud_topics, {"points"}); // '/d435_Down/depth/color/points'
  nh.getParam("publishers", publishers);  // elevation_map_filter
  nh.param<std::vector<std::string>>("initialize_frame_id", initialize_frame_id_, {"base"});  // initialize_frame_id: ['RF_FOOT', 'LF_FOOT', 'RH_FOOT', 'LH_FOOT']
  ...
  {
    //! subscribe camera & LIDAR pointclound
  ...
  {
    // parse params
    std::string topic_name = itr->first;  // elevation_map_filter
    std::vector<std::string> layers_list;
    std::vector<std::string> basic_layers_list; 
    auto layers = itr->second["layers"]; // ['min_filter', 'smooth', 'inpaint', 'upper_bound']
    auto basic_layers = itr->second["basic_layers"];  // ['min_filter']
    double fps = itr->second["fps"];  // 5.0
...
// void ElevationMappingNode::pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud) 
{
...
{
  // std::cout << "[initializeWithTF]" << std::endl;
  ...
  for (const auto& frame_id : initialize_frame_id_) 
  {
    ...
    try 
    {
      ROS_INFO_STREAM("Initializing " << frame_id);
  ...
  std::cout << "initializeWithTF finished" << std::endl;
}
