...
{
  //! 在世界坐标系下，计算所有region和positionInWorld的距离并进行排列
  ...
  for (const auto& regionInfo : sortedRegions)  // 遍历所有的region
  {
    ...
    if (cost < projection.cost) 
    {
      ...
      projection.positionInTerrainFrame = projectedPointInTerrainFrame; //! 这个不太对阿
